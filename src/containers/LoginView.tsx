import { Button, cn, Grid, Input, Typography } from '@flow/core';
import React, { type FormEvent, useEffect, useState } from 'react';

// TODO: change image
import loginImage from '~/assets/image 19.png';
...
const LoginView: React.FC = () => {
  ...
  return (
    <Grid
      container
      className="flex flex-1 flex-col-reverse items-center justify-center lg:grid lg:max-w-none"
    >
      <Grid
        columns={{
          sm: 6,
      ...
      >
        <div className="min-h-0 rounded-3xl border border-slate-7 px-xl py-4xl shadow-xs">
          {step === 'email' ? (
            <EmailForm
              email={email}
              setEmail={setEmail}
...
const EmailForm = ({
  ...
  <form onSubmit={onSubmit}>
    ...
    <div className="flex items-baseline gap-2.5">
      ...
      <Button type="submit" className="rounded-full text-iris-1">
        Continue
      </Button>
    </div>
  </form>
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <div className={cn('my-2.5 flex gap-2.5', invalid ? 'items-baseline' : 'items-center')}>
        ...
        <Button type="submit" className="rounded-full bg-blue-9 text-iris-1">
          Sign in
        </Button>
      </div>
